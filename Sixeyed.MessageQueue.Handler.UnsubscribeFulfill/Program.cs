...
namespace Sixeyed.MessageQueue.Handler.UnsubscribeFulfill
{
    class Program
    {
        static void Main(string[] args)
        {
            using (var queue = new msmq.MessageQueue(".\\private$\\sixeyed.messagequeue.unsubscribe-fulfillment"))
            {
                queue.MulticastAddress = "234.1.1.2:8001";
                while (true)
                {
                    Console.WriteLine("unsubscribe-fulfillment listening");
                    ...
                    if (body.GetType() == typeof(UserUnsubscribed))
                    {
                        ...
                        var workflow = new UnsubscribeFulfillmentWorkflow(unsubscribedEvent.EmailAddress);
                        workflow.Run();
